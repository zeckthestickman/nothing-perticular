...
// FIXME ---- REMOVE THIS ----
class al::HitInfo {};
class al::ArrowHitInfo : public al::HitInfo {};
// ---------------------------
...
void PlayerJudgeWallKeep::update() {
    ...
    if (mModelChanger->is2DModel() || mCounterForceRun->isForceRun() ||
        mCounterAfterUpperPunch->getCounter() < mConst->getWallInhibitAfterPunch() ||
    ...
    // required to be explicit block to match function return pattern
    {
        ...
        sead::Vector3f nearWallPosition = rs::getCollidedWallPos(mCollider) - collidedWallNormal * 5.0f;
        ...
        if (alCollisionUtil::getLastPolyOnArrow(mPlayer, &hitInfo1, nearWallPosition + sideOffset, -sideOffset, nullptr,
                                                &filterWallOnly)) {
            const sead::Vector3f& hitPos = alCollisionUtil::getCollisionHitPos(hitInfo1);
            const sead::Vector3f& hitNormal = alCollisionUtil::getCollisionHitNormal(hitInfo1);
        ...
        if (alCollisionUtil::getLastPolyOnArrow(mPlayer, &hitInfo2, nearWallPosition - sideOffset, sideOffset, nullptr,
                                                &filterWallOnly)) {
            const sead::Vector3f& hitPos = alCollisionUtil::getCollisionHitPos(hitInfo2);
            const sead::Vector3f& hitNormal = alCollisionUtil::getCollisionHitNormal(hitInfo2);
            if (!rs::calcExistCollisionBorder(mPlayer, hitPos, hitNormal)) {
                if (sead::Mathf::abs(al::calcAngleOnPlaneDegree(
                        hitNormal, collidedWallNormal, gravity2)) > mConst->getWallFollowAngleH())
                    return;
